...
```json
{
  ...
  "params": {
    "name": "links_upsert",
    "arguments": { "slug": "summer-sale", "payload": { "title": "Summer Sale", "html": "<html>..." } }
  }
}
```
...
## Upload Modes (Legacy — via links_upsert)

> **NOTE**: Page hosting via `links_upsert` is deprecated. Use the site-deployments Direct API above instead. `links_upsert` is for redirect links only.

The Link Platform previously supported upload modes via `links_upsert`:

1. **Single HTML** — payload with `html` field
   - `links_upsert({ slug, payload: { title: "...", html: "<full page>" } })`
   - Best for: single-page sites with inline CSS/JS

2. **Multi-File** — payload with `files` array
   - `links_upsert({ slug, payload: { title: "...", files: [{path: "index.html", content: "..."}, {path: "styles.css", content: "..."}] } })`
   - Best for: pages with separate CSS/JS files

3. **Folder Upload** — payload with `folder` path
   - `links_upsert({ slug, payload: { title: "...", folder: "./build/" } })`
   - Best for: full project builds with images/fonts/assets

Use exactly ONE of `html`, `files`, or `folder` in the payload per call.
...
1. **Wrong tool name**: Use Link Platform names (e.g., `links_upsert` for page creation)
2. **Missing payload**: Most write tools require a `payload` object, not flat params
3. **Expired PAT**: JWT tokens expire every 1 hour — watch for 401 errors
4. **Lost session**: Always store `mcp-session-id` and reuse it
5. **Wrong URL**: Each user has their own server at `https://{handle}.mcp.10x.in/mcp`
6. **Missing Accept header**: Must include `application/json, text/event-stream`
7. **Using `links_upsert` for page hosting**: `links_upsert` creates REDIRECT LINKS (302) only. The `html` parameter is accepted but IGNORED. Use the site-deployments Direct API for page hosting.
8. **Wrong auth for site-deployments**: Site-deployments requires a JWT (Cognito ID token). PAT (`patv1_*`) will NOT work.
9. **Passing large HTML through MCP**: The MCP proxy has a ~1KB payload limit. Use the Direct API for page content.
