...
2. **Extract state** automatically:
   ...
   - `links_upsert` (page creation) → add to `pages` map
   - `links_upsert` (publish) → update page status to "published"
   - `links_upsert` (link creation) → add to `links` map
...
```json
{
  ...
  "pages": { "summer-sale": { "url": "https://anit.10x.in/summer-sale", "action": "published" } },
  "links": { "lnk_x": { "slug": "/go/summer", "action": "created" } },
...
```json
{
  ...
  "lastSession": {
    "date": "2026-02-22",
    "duration": "45 min",
    "accomplished": ["Published 'Summer Sale' at anit.10x.in/summer-sale"],
