...
| **Deploy** | `links_upsert`, `agent_rollback_run` | QA Director / Technical Lead |
| **Quality** | `system_audit_events` | Testing Agent |
| **Links** | `links_upsert`, `links_list` | Campaign Manager |
| **Pages** | `links_upsert`, `links_list` | Campaign Manager / Technical Lead |
...
1. **Draft** → `agent_create_proposal` creates strategy record
2. **Preview** → `agent_start_run` → server tests and returns preview
3. **Publish** → `links_upsert` (with publish payload) → live at `{handle}.10x.in/{slug}`
...
Growth Strategist → agent_create_proposal (create strategy)
    ...
    → Campaign Manager (SEO + analytics + links_upsert)
    → agent_start_run → server tests → user reviews preview
    → User decides: publish / save draft / discard
    → Launch: links_upsert (with publish payload)
```
...
Testing Agent (full audit + WebMCP verification)
    ...
    → Campaign Manager (SEO + tracking + links_upsert)
    → links_upsert (with publish payload)
