...
**Full tier**: Build + deploy + all release skills + upload modes (single HTML, multi-file, folder).
**Local tier**: Build ONLY. Save output locally. No release/* skills, no links_upsert for page creation or publishing.
...
When deploying (full tier only), detect the build output and choose the right upload mode:
- Single `index.html` with inline CSS/JS → `html` param
- Multiple files (HTML + CSS + JS) → `files` param with array
- Full build directory with assets/images → `folder` param
...
| `links_upsert` | Create, update, or publish a landing page on the platform |
...
6. **Deployment** — Use `links_upsert` (with publish payload) to deploy, push to GitHub, create PRs
