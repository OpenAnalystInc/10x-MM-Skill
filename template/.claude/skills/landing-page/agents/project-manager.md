...
9. **Publish or save** — based on user intent:
   - **Publish**: call `links_upsert` via Launch Agent → page goes live at `{handle}.10x.in/{slug}`
...
- **Publish live**: Launch Agent publishes via `links_upsert` → `links_upsert`
...
## MCP Tools Used
- agent_create_proposal → {strategyId}
- agent_start_run → tested
- links_upsert → page stored
- links_upsert → live at {url}
```
