...
Detect the build output and choose the appropriate upload mode:

1. Check `projects/{name}/build/` directory
2. If only `index.html` exists → Single HTML mode (`html` param)
3. If multiple files (HTML + CSS + JS) → Multi-file mode (`files` param)
4. If directory has images/fonts/assets → Folder upload mode (`folder` param)

Pass the appropriate param to `links_upsert` and `links_upsert`.
...
8. **Publish via MCP tools** — use these tools in order:
   a. `links_upsert` — upload HTML content to platform
   b. `links_upsert` — make page live at `{handle}.10x.in/{slug}`
   c. `links_upsert` — create campaign links pointing to the page
   d. `system_audit_events` — verify all links and content post-publish
...
**Primary (10x.in Platform — via MCP tools)**:
1. Call `links_upsert` with the full HTML content
2. Call `links_upsert` to make it live
3. Call `links_upsert` to create campaign tracking links
4. Verify: page is live at `{handle}.10x.in/{slug}`
