...
| Site-deployments API | `POST /v2/handles/{handle}/site-deployments` with `{"inlineHtml": "<html>"}` — deploys page (JWT auth, auto-activates) |
...
5. **Release Management** — Create branches, preview, PR review, publish (via site-deployments API), rollback
